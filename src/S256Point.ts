...
import {encodeBase58Checksum, hash160, mod, pow, toBufferBE, toBufferLE} from "./helper";
...
//        u*G + v*P should have as the x coordinate, r
    ...
    sec(compressed: boolean = true): Buffer {
        if (compressed) {
            if (this.y!.num % 2n === 0n) {
                return Buffer.concat([Buffer.alloc(1, 0x02), toBufferBE(this.x!.num, 32)]);
            } else {
                return Buffer.concat([Buffer.alloc(1, 0x03), toBufferBE(this.x!.num, 32)]);
            }
        } else {
            return Buffer.concat([Buffer.alloc(1, 0x04), toBufferBE(this.x!.num, 32), toBufferBE(this.y!.num, 32)]);
        }
    }
