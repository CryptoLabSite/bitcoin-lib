...
export class FinitePoint {
  constructor(
    ...
    if (
      this.y
        .pow(2n)
        .nonEquals(this.x.pow(3n).add(this.a.mul(this.x)).add(this.b))
    ) {
  ...
  toString() {
    ...
    return `FinitePoint(${this.x!.num},${this.y!.num})_${this.a.num}_${
      this.b.num
    }`;
  }
  ...
  add(other: FinitePoint): FinitePoint {
    if (this.a.nonEquals(other.a) || this.b.nonEquals(other.b)) {
      throw new Error(
        `FinitePoint ${toString()}, ${other.toString()} are not on the same curve.`,
      );
    }
